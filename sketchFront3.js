...
function displayCurrentEntry() {
	...
	if (images[imageFileName]) {
		...
		image(images[imageFileName], 26, 226, 323, 323); // Draw the image
		pop();
	}
	...
	push();
	fill(fillColor);
	rect(25, 25, 1150, 200); // Name rectangle
	pop();
...
function drawShapesGrid(scale, region, dept, density, fillColor, gridRows) {
	...
	let gridCols = Math.ceil((895 / 325) * gridRows); // Calculate columns to maintain square grid within the reduced rectangle
	let cellSize = baseDiameter + spacing;

	// Apply buffer to region dimensions
	let buffer = 24;
	let regionX = 350 + buffer - (70 * 1) / gridRows;
	let regionY = 550 + buffer;
	let regionWidth = 825 - 2 * buffer;
	let regionHeight = 325 - 2 * buffer;
